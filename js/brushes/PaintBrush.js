...
export default class PaintBrush extends Brush {
  ...
  render(ctx, obj) {
    ...
    // ── Blend mode: pre-compute smooth carry-colour per sample ──
    let blendRGB = null;
    if (blendColors) {
      let seed = null;
      ...
      if (seed) {
        const mixRate = 0.3;
        const lookback = Math.max(1, Math.floor(blendColors.length * 0.06));
        const carry = { r: seed.r, g: seed.g, b: seed.b };
        blendRGB = new Array(stepCount + 1);
        for (let si = 0; si <= stepCount; si++) {
          const t = si / stepCount;
          const rawCi = Math.floor(t * blendColors.length) - lookback;
          const ci = Math.max(0, Math.min(rawCi, blendColors.length - 1));
          ...
          blendRGB[si] = { r: Math.round(carry.r), g: Math.round(carry.g), b: Math.round(carry.b) };
        }
      }
    }
    ...
    if (blendRGB) {
      // ── Blend mode ──
      // ── Blend: soft-stamp smudge with bristle texture overlay ──
      // Phase 1: smooth colour stamps along the path
      const stampAlpha = 0.45;
      for (let si = 0; si <= stepCount; si++) {
        const s = samples[si];
        const stampRadius = halfW * Math.max(0.3, s.spread);
        if (stampRadius < 0.5) continue;
        const sc = blendRGB[si];
        const cr = sc.r, cg = sc.g, cb = sc.b;
        const grad = oc.createRadialGradient(s.x, s.y, 0, s.x, s.y, stampRadius);
        grad.addColorStop(0, `rgba(${cr},${cg},${cb},${stampAlpha})`);
        grad.addColorStop(0.55, `rgba(${cr},${cg},${cb},${(stampAlpha * 0.45).toFixed(3)})`);
        grad.addColorStop(1, `rgba(${cr},${cg},${cb},0)`);
        oc.fillStyle = grad;
        oc.beginPath();
        oc.arc(s.x, s.y, stampRadius, 0, Math.PI * 2);
        oc.fill();
      }

      ctx.globalAlpha = opacity;
      ctx.drawImage(off.c, ox, oy);
    } else {
      // ── Normal paint: draw bristles + blur pass ──
      ...
      for (let bi = 0; bi < bristleCount; bi++) {
        ...
        const edgeDist = Math.abs(bt);
        const tipShape = head.edgeSoftness;
        const profile = 1 - Math.sqrt(Math.max(0, 1 - edgeDist * edgeDist));
        const startDelay = profile * tipShape * halfW * 2.0 + rng() * halfW * 0.35;
        const endLift = profile * tipShape * halfW * 1.5 + rng() * halfW * 0.25;
